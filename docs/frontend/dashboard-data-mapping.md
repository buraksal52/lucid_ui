...
| `lucidui.raw.contrast`, `.elements`, `.groups`, `.textDensity` | `MetricDetailDrawer` | Exact engine field names (e.g. `contrast.averageContrastRatio`, `elements.interactiveTargetCount`) — see [metric-catalog.md](../metrics/metric-catalog.md). `clutter` and `whitespaceAlignment` no longer exist (`corrected-v4`, Tier 3 removal). Each remaining field already has its formatted counterpart in `presentation.metricSections`. |
...
| `lucidui.normalized` | `MetricDetailDrawer` | Only 2 keys exist as of `corrected-v4`: `contrast`, `textDensity` — there is no normalized form for elements, grouping, colorfulness, Fitts's Law, or visual balance (`presentation.metricSections[].normalizedScore` is `null` for those, by design, not by omission). |
