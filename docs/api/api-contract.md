...
> **Phase 4 status**: `POST /api/v1/analyses/single` now implements the full request/response contract documented below, with one remaining gap: `comparison.agreementLevel` is always `unavailable` — agreement/discrepancy computation is Phase 6 (see [ROADMAP.md](../../ROADMAP.md)). `lucidui` is real, computed output from the deterministic metric engine — see [docs/metrics/scoring-and-normalization.md](../metrics/scoring-and-normalization.md). `llmInterpretation` and `uiclip` are both real — see "LLM Interpretation" and "UIClip Evaluation" below. `uiclip` currently only ever uses the offline mock evaluator (`UICLIP_PROVIDER=mock`); no official/real UIClip model is integrated yet (Phase 5).
...
`uiclip` is populated by `app.uiclip.UIClipEvaluationService` (see ROADMAP.md Phase 4). It is an **independent learned evaluator** — never ground truth, never merged with or reinterpreting LucidUI's deterministic metrics (see [ADR-004](../architecture/decisions/ADR-004-uiclip-independent-evaluator.md)). It receives only the decoded screenshot and a description string — never `lucidui`, `llmInterpretation`, or comparison results.

- **Provider selection** is configuration-driven (`UICLIP_PROVIDER` env var; see `backend/.env.example`), not hardcoded. `mock` (default, and the only implemented value as of Phase 4) is a deterministic, offline evaluator — no model download, no network call, always returns `status: "completed"` with `modelVersion: "mock-uiclip-v1"`. No official/real provider exists yet — see [docs/research/uiclip-integration.md](../research/uiclip-integration.md) for the verified findings on official model availability and why real integration is Phase 5, not this phase.
- **`runUiclip`** (request field, default `true`) controls whether this stage runs at all: `false` → `uiclip.status: "disabled"`, `uiclip.enabled: false`, matching the status catalog in [report-schema.md](report-schema.md#uiclip-statuses-uiclipstatus).
- **`description`** resolves to `descriptionSource: "user"` when a non-blank value is submitted, or the already-documented `descriptionSource: "generic"` fallback (`"A software user interface screenshot."`) when omitted or blank — see [report-schema.md](report-schema.md#description-sources-uiclipdescriptionsource-and-anywhere-a-description-is-attached). Never a silently invented one-off string.
- **`normalizedQualityScore` is always `null`**: the official UIClip paper (arXiv:2404.12500) computes its score as the dot product between image and text embeddings — an uncalibrated, CLIP-style similarity/logit value. No documented, independently-verified 0-100 or 0-1 normalization was confirmed from a primary source, so none is invented here (see CLAUDE.md — "Do not invent a 0-100 normalization"). `qualityScore` carries the raw score instead.
- **Failure handling**: if no provider is configured/available, `status` becomes `"unavailable"`. If the provider runs but its output is malformed or fails validation, `status` becomes `"failed"`. In both cases the rest of the report (deterministic metrics, LLM interpretation, image metadata) is still returned and persisted — a UIClip failure never discards the rest of the analysis, and never surfaces as a top-level HTTP error (`uiclip.status` carries the detail instead).
