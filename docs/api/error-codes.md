...
| `ANALYSIS_FAILED` | 500 | The deterministic metric engine failed to produce a report. OCR execution failure alone does not produce this error; the engine continues with empty OCR data for OCR-dependent fields. No usable report body is returned. | `null`, or a short diagnostic string — never raw internal stack traces. |
